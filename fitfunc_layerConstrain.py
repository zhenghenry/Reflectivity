...
def loadParams(paramsFile):
    ...
    for l in range(5, len(params[0, :]), 3):
        for i, k in enumerate(('r', 'd', 's')):
            paramsDict['%s%d' % (k, ((l - 5) / 3) + 1)] = pyRefFit.parameter(params[0, l + i], params[1, l + i],
                                                                             params[2, l + i], False)
...
def reflectivity(q, params, isSLD=False):
    ...
    for n in range(0, len(params), 3):
        if params[n] >= 0:
            ...
            rho = params[int(abs(params[n]) - 5)]
        if params[n + 1] >= 0:
            d = params[n + 1]
        else:
            d = params[int(abs(params[n + 1]) - 5)]
        if params[n + 2] >= 0:
            sig = params[n + 2]
        else:
            sig = params[int(abs(params[n + 2]) - 5)]

        layers.append((rho, d, sig))  # (tuple(params[n:n+3]))
    ...
    for n in range(0, len(layers) - 1):
        depth += layers[n][1]
        r += (layers[n][0] - layers[n + 1][0]) * sldConversion * np.exp(1j * q * depth) * np.exp(
            -q ** 2 * layers[n + 1][2] ** 2 / 2)
...
def density(z, params, roughness=True):
    ...
    for n in range(0, len(layers) - 1):
        ...
        if roughness:
            density += (layers[n + 1][0] - layers[n][0]) * (
            1 + scipy.special.erf((z - depth) / np.sqrt(2) / layers[n + 1][2])) / 2
        else:
            density += (layers[n + 1][0] - layers[n][0]) * (
            1 + scipy.special.erf((z - depth) / np.sqrt(2) / 0.0001)) / 2
